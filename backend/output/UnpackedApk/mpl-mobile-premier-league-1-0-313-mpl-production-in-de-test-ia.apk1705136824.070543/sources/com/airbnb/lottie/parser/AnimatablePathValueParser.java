...
public class AnimatablePathValueParser {
    public static final Options NAMES = Options.of(k.f4287a, "x", "y");
    ...
    public static AnimatablePathValue parse(JsonReader jsonReader, LottieComposition lottieComposition) throws IOException {
        ...
        if (jsonReader.peek() == Token.BEGIN_ARRAY) {
            ...
            while (jsonReader.hasNext()) {
                arrayList.add(new PathKeyframe(lottieComposition, KeyframeParser.parse(jsonReader, lottieComposition, Utils.dpScale(), PathParser.INSTANCE, jsonReader.peek() == Token.BEGIN_OBJECT, false)));
            }
            jsonReader.endArray();
            ...
            if (selectName != 0) {
                if (selectName != 1) {
                    if (selectName != 2) {
                        ...
                        animatableFloatValue2 = co.hyperverge.hypersnapsdk.c.k.parseFloat(jsonReader, lottieComposition);
                    }
                } else if (jsonReader.peek() == Token.STRING) {
                    jsonReader.skipValue();
                } else {
