...
public enum c {
    ...
    FAQ_NOT_FOUND(BaseRequest.AddBuddy, "Oops! Unable to display linked content"),
    LINKED_CONTENT_NOT_FOUND(202, "Oops! Unable to display linked content"),
    ERROR_TAGS_COUNT_EXCEEDED(BaseRequest.RemoveBuddy, "Limit exceeded: Maximum number of filter tag(s) allowed is 25."),
    VOICE_MESSAGE_RECORDING_FAILED(BaseRequest.InvitationReply, "Failed to record voice message. Please try later"),
    ...
    INVALID_FRESHCHAT_CONFIG(FTPReply.SYNTAX_ERROR_IN_ARGUMENTS, "Freshchat.init() requires a valid FreshchatConfig instance"),
    INVALID_FRESHCHAT_CONFIG_NON_FRESCHAT_DOMAIN(FTPReply.COMMAND_NOT_IMPLEMENTED, "Potentially misconfigured domain for Freshchat. Got {{domain}} as domain. Would typically be of the format something.freshchat.com"),
    INVALID_FRESHCHAT_CONFIG_APP_ID(FTPReply.BAD_COMMAND_SEQUENCE, "Freshchat.init() requires a valid FreshchatConfig instance - App Id is missing or invalid"),
    INVALID_FRESHCHAT_CONFIG_APP_KEY(FTPReply.COMMAND_NOT_IMPLEMENTED_FOR_PARAMETER, "Freshchat.init() requires a valid FreshchatConfig instance - App Key is missing or invalid"),
    ...
    ERROR_WHILE_CROPPING(FTPReply.FILE_UNAVAILABLE, "Exception while performing circular crop"),
    ...
    USER_PROPERTY_EMAIL_ERROR(SocialShareModule.SHARE_TO_WHATSAPP, "Email is not valid."),
    USER_PROPERTY_EXTERNAL_ID_ERROR(1004, "External Id should be maximum of 256 charecters."),
    USER_PROPERTY_COUNTRY_CODE_ERROR(WebSocketProtocol.CLOSE_NO_STATUS_CODE, "Phone number country code should be maximum of 6 digits including +."),
    USER_PROPERTY_PHONE_NUMBER_ERROR(1006, "Phone number should be maximum of 16 charecters and can contain digits, +, ., (, ), -."),
    USER_PROPERTY_RESTORE_ID_ERROR(Constant.REQUEST_CODE_FOR_SHORT_CUT, "Invalid restore id"),
    USER_NOT_REGISTERED_ACTIVITY_NOT_SENT(Constant.REQUEST_ID_READ_PHONE_STATE, "User not registered. User activity ignored."),
