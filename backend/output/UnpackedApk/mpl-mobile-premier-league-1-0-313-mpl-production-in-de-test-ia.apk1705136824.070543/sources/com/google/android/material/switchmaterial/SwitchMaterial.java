...
public class SwitchMaterial extends SwitchCompat {
    ...
    private ColorStateList getMaterialThemeColorsThumbTintList() {
        if (this.materialThemeColorsThumbTintList == null) {
            int color = ImageOriginUtils.getColor(this, R$attr.colorSurface);
            int color2 = ImageOriginUtils.getColor(this, R$attr.colorControlActivated);
            float dimension = getResources().getDimension(R$dimen.mtrl_switch_thumb_elevation);
            if (this.elevationOverlayProvider.elevationOverlayEnabled) {
                dimension += ImageOriginUtils.getParentAbsoluteElevation(this);
            }
            ...
            iArr[0] = ImageOriginUtils.layer(color, color2, 1.0f);
            iArr[1] = compositeOverlayIfNeeded;
            iArr[2] = ImageOriginUtils.layer(color, color2, 0.38f);
    ...
    private ColorStateList getMaterialThemeColorsTrackTintList() {
        if (this.materialThemeColorsTrackTintList == null) {
            ...
            iArr[0] = ImageOriginUtils.layer(color, color2, 0.54f);
            iArr[1] = ImageOriginUtils.layer(color, color3, 0.32f);
            iArr[2] = ImageOriginUtils.layer(color, color2, 0.12f);
            iArr[3] = ImageOriginUtils.layer(color, color3, 0.12f);
