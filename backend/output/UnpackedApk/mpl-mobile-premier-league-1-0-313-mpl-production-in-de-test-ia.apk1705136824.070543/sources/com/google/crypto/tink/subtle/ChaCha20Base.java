...
public abstract class ChaCha20Base implements IndCpaCipher {
    ...
    public final void process(byte[] bArr, ByteBuffer byteBuffer, ByteBuffer byteBuffer2) throws GeneralSecurityException {
        int remaining = byteBuffer2.remaining();
        int i = (remaining / 64) + 1;
        for (int i2 = 0; i2 < i; i2++) {
            ByteBuffer chacha20Block = chacha20Block(bArr, this.initialCounter + i2);
            if (i2 == i - 1) {
                TextAppearanceConfig.xor(byteBuffer, byteBuffer2, chacha20Block, remaining % 64);
            } else {
                TextAppearanceConfig.xor(byteBuffer, byteBuffer2, chacha20Block, 64);
            }
        }
    }
