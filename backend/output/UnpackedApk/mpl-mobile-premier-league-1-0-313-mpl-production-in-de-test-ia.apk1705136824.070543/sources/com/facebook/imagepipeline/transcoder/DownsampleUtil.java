...
public class DownsampleUtil {
    ...
    public static float determineDownsampleRatio(RotationOptions rotationOptions, ResizeOptions resizeOptions, EncodedImage encodedImage) {
        k.checkArgument(EncodedImage.isMetaDataAvailable(encodedImage));
        ...
        FLog.v((String) "DownsampleUtil", (String) "Downsample - Specified size: %dx%d, image size: %dx%d ratio: %.1f x %.1f, ratio: %.3f", Integer.valueOf(resizeOptions.width), Integer.valueOf(resizeOptions.height), Integer.valueOf(height), Integer.valueOf(width), Float.valueOf(f2), Float.valueOf(f3), Float.valueOf(max));
        return max;
    }
    ...
    public static int ratioToSampleSize(float f2) {
        ...
        while (true) {
            double d2 = (double) i;
            if (((1.0d / (Math.pow(d2, 2.0d) - d2)) * 0.3333333432674408d) + (1.0d / d2) <= ((double) f2)) {
                return i - 1;
            }
            i++;
        }
    ...
    public static int ratioToSampleSizeJPEG(float f2) {
        ...
        while (true) {
            ...
            if ((0.3333333432674408d * d2) + d2 <= ((double) f2)) {
                return i;
            }
            i = i2;
        }
