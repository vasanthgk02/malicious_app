...
public final class SafeCollector<T> extends ContinuationImpl implements FlowCollector<T>, CoroutineStackFrame {
    ...
    public final Object emit(Continuation<? super Unit> continuation, T t) {
        ...
        TypeUtilsKt.ensureActive(context);
        CoroutineContext coroutineContext = this.lastEmissionContext;
        if (coroutineContext != context) {
            if (coroutineContext instanceof DownstreamExceptionContext) {
                StringBuilder outline73 = GeneratedOutlineSupport.outline73("\n            Flow exception transparency is violated:\n                Previous 'emit' call has thrown exception ");
                outline73.append(((DownstreamExceptionContext) coroutineContext).f5954e);
                outline73.append(", but then emission attempt of value '");
                outline73.append(t);
                outline73.append("' has been detected.\n                Emissions from 'catch' blocks are prohibited in order to avoid unspecified behaviour, 'Flow.catch' operator can be used instead.\n                For a more detailed explanation, please refer to Flow documentation.\n            ");
                throw new IllegalStateException(CharsKt__CharKt.trimIndent(outline73.toString()).toString());
            } else if (((Number) context.fold(Integer.valueOf(0), new SafeCollector_commonKt$checkContext$result$1(this))).intValue() == this.collectContextSize) {
                this.lastEmissionContext = context;
            } else {
                StringBuilder outline732 = GeneratedOutlineSupport.outline73("Flow invariant is violated:\n\t\tFlow was collected in ");
                outline732.append(this.collectContext);
                outline732.append(",\n\t\tbut emission happened in ");
                outline732.append(context);
