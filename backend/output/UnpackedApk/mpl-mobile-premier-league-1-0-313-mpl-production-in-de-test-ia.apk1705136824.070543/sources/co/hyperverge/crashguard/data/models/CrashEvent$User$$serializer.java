...
public final class CrashEvent$User$$serializer implements GeneratedSerializer<User> {
    ...
    public void serialize(Encoder encoder, User user) {
        ...
        Intrinsics.checkNotNullParameter(user, HSLCriteriaBuilder.VALUE);
        SerialDescriptor descriptor2 = getDescriptor();
        ...
        boolean z = true;
        if (!beginStructure.shouldEncodeElementDefault(descriptor2, 0) && user.id == null) {
            z = false;
        }
        if (z) {
            beginStructure.encodeNullableSerializableElement(descriptor2, 0, StringSerializer.INSTANCE, user.id);
        }
        beginStructure.endStructure(descriptor2);
    }
