...
public class ResChangeRoomCapacity implements IResHandler {
    public void handleResponse(ISmartFox iSmartFox, SystemController systemController, IMessage iMessage) throws Exception {
        ...
        if (!content.containsKey(BaseRequest.KEY_ERROR_CODE)) {
            ...
            if (roomById != null) {
                iSmartFox.getRoomManager().changeRoomCapacity(roomById, content.getInt("u").intValue(), content.getInt("s").intValue());
                hashMap.put("room", roomById);
                iSmartFox.dispatchEvent(new SFSEvent(SFSEvent.ROOM_CAPACITY_CHANGE, hashMap));
                return;
            }
            Logger logger = iSmartFox.getLogger();
            logger.warn("Room not found, ID:" + intValue + ", Room capacity change failed.");
            return;
        }
        ...
        GeneratedOutlineSupport.outline98(SFSEvent.ROOM_CAPACITY_CHANGE_ERROR, hashMap, iSmartFox);
    }
}
