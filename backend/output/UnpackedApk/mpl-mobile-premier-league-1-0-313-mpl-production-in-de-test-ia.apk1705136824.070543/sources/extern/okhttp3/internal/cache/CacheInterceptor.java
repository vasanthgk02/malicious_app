...
import extern.okhttp3.internal.cache.CacheStrategy.Factory;
...
public final class CacheInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (request == null && response2 == null) {
            return new Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(FTPReply.COMMAND_NOT_IMPLEMENTED_FOR_PARAMETER).message("Unsatisfiable Request (only-if-cached)").body(Util.EMPTY_RESPONSE).sentRequestAtMillis(-1).receivedResponseAtMillis(System.currentTimeMillis()).build();
        }
        if (request == null) {
    ...
    private static Headers combine(Headers headers, Headers headers2) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ((!Names.WARNING.equalsIgnoreCase(name) || !value.startsWith("1")) && (isContentSpecificHeader(name) || !isEndToEnd(name) || headers2.get(name) == null)) {
    ...
    static boolean isEndToEnd(String str) {
        return !"Connection".equalsIgnoreCase(str) && !"Keep-Alive".equalsIgnoreCase(str) && !"Proxy-Authenticate".equalsIgnoreCase(str) && !Names.PROXY_AUTHORIZATION.equalsIgnoreCase(str) && !Names.TE.equalsIgnoreCase(str) && !"Trailers".equalsIgnoreCase(str) && !Names.TRANSFER_ENCODING.equalsIgnoreCase(str) && !"Upgrade".equalsIgnoreCase(str);
    }
